...
def readdata():
    ...
    print('allobstypes=', allobstypes)
    print('obs_prefix=', obsfiles_prefix)
    ...
    for file_name in obsfiles:
        ...
        for var in obslist:
            ...
            if (var_name == 'refractivity' or var_name == 'atmosphericRefractivity'):
                var_unit = 'N-unit'
            elif (var_name == 'bending_angle' or var_name == 'Bending Angle'):
                var_unit = 'Radians'
            else:
            ...
            if channels[0] == vu.miss_i:
                ...
                stationidnc_array = []
                recordnc_array = []
                if (obstype == 'gnssro' or obstype == 'gnssroref' or 
                    obstype == 'gnssrobndmo'  or obstype == 'gnssrobndmo-nopseudo' or 
                    obstype == 'gnssrobndnbam' or obstype == 'gnssrobndropp1d'  
                    or obstype == 'gnssrorefncep'):
                ...
                if ((obstype == 'gnssro' or obstype == 'gnssroref'or 
                    obstype == 'gnssrobndmo'  or obstype == 'gnssrobndmo-nopseudo' or 
                    obstype == 'gnssrobndnbam' or obstype == 'gnssrobndropp1d'  
                    or obstype == 'gnssrorefncep') and test == 'cycling'):
                ...
                channel_values = nc["Channel"][:]
                # Channels to plot 
                target_values = channels 
                # Find the indexes of thoses channels 
                indices = [np.where(channel_values == value)[0][0] for value in target_values if value in channel_values]
                print("positions of channels in the dataset:", indices)
                print ("Channels to Plot :", channels)
                for channel in indices:
                    print ("Index: ",channel,"  channel n°:",channel_values[channel])
                    ...
                    var_name = var_name +'_ch'+ str(channel_values[channel])
                    nstation = 0
                    basic_plot_functions.plotDistri(latnc,lonnc,obsnc,obs_type,var_name,var_unit,out_name,nstation,levbin)
                    var_name = var[9:]
def main():
